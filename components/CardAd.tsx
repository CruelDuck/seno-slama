      ...
      <svg width="56" height="56" viewBox="0 0 64 64" aria-hidden="true">
        <defs>
          <linearGradient id="g" x1="0" x2="1">
            <stop offset="0%" stopColor={gold ? '#fde68a' : '#bbf7d0'} />
...
export default function CardAd({ ad }: { ad: Ad }) {
  ...
  const ring = ad.typ_inzeratu === 'Nabídka' ? 'card-offer' : 'card-demand'

  return (
    <a className={`card ${ring} block overflow-hidden transition hover:shadow-md`} href={`/inzerat/${ad.id}`}>
      ...
      <div className="p-4 space-y-2">
        <div className="flex items-center gap-2 text-xs">
          <span className="badge">{ad.typ_inzeratu}</span>
          <span className="badge">{ad.produkt}</span>
          <span className="ml-auto text-zinc-500">{ad.okres ? `${ad.kraj} • ${ad.okres}` : ad.kraj}</span>
        </div>
        <h3 className="font-semibold leading-snug line-clamp-2">{ad.nazev}</h3>
