...
async function getByToken(token: string, sb = supabaseService()) {
  ...
  const { data: ad } = await sb.from('inzeraty').select('id,expires_at').eq('id', t.inzerat_id).single()
...
export async function POST(req: NextRequest) {
  ...
  const { error } = await sb.from('inzeraty').update({ expires_at: new Date(Date.now()+30*864e5).toISOString(), status: 'Ověřeno' }).eq('id', ad.id)
